...
$stmt = $conn->prepare("SELECT * FROM tbl_game_product WHERE game_details_id = 2");
$stmt->execute();
                    ...
                    <?php foreach ($products as $product): ?>
                        <div class="product">
                            <div class="inner-product">
                                ...
                                <a href="details-genshin.php?product_img=<?php echo urlencode($product['product_img']); ?>&game_product_name=<?php echo urlencode($product['game_product_name']); ?>&game_product_details=<?php echo urlencode($product['game_product_details']); ?>&game_product_price=<?php echo urlencode($product['game_product_price']); ?>" class="button">Buy now</a>
                            </div>
                        </div>
                    <?php endforeach; ?>
                </div>
