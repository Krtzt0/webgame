...
if (isset($_POST['action']) && isset($_POST['order_id'])) {
    ...
    switch ($action) {
        case 'accept':
            updateOrderStatus($order_id, 2); // 2 คือ รับงาน
            break;
        case 'complete':
            updateOrderStatus($order_id, 3); // 3 คือ เสร็จสิ้น
            break;
        default:
...
function updateOrderStatus($order_id, $status) {
    global $conn;
    try {
        $sql = "UPDATE tbl_orders SET status = :status WHERE orders_id = :order_id";
                    ...
                    <tbody>
                        ...
                        try {
                            ...
                            $sql = "SELECT o.orders_id, g.game_details_name, o.game_product, o.workdetail, 
                                    ...
                                    WHERE o.game_details_id = 2 AND o.status = 3";
                            $stmt = $conn->prepare($sql);
                            $stmt->execute();
                            ...
                            if ($stmt->rowCount() > 0) {
                                ...
                                foreach ($result as $row) {
                                    ...
                                    // แก้ไขที่อยู่รูปภาพและ HTML syntax
